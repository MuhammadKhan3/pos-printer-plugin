...
function print_handler(printer_conn, print_data, order_data, date_time, template_style, print_data_other, attr_id_main, attr_arr, runner_flag) {
    ...
    var show_other_stations = false;

    var show_other_stations_extras = false;
    ...
    if (order_data.table_name && order_data.table_name != '' && order_data.table_name.trim()!="<>" && order_data?.table_name?.includes("undefined")!==true) {
    ...
    if (template_styles[template_style - 1].show_other_stations_extras != undefined) {
        show_other_stations_extras = template_styles[template_style - 1].show_other_stations_extras;
    }
...
if (order_data.table_name && order_data.table_name != '' && order_data.table_name.trim()!="<>" && order_data?.table_name?.includes("undefined")!==true) {
    ...
    print_data.forEach(function(order_info) {
        ...
        // console.log(print_item_flag, runner_flag, isItemIdInArray(attr_id_main, order_info.attribute_id, runner_flag), attr_id_main, order_info.attribute_id);

       

        if((print_item_flag === true) || ((runner_flag === true) && (isItemIdInArray(attr_id_main, order_info.attribute_id, runner_flag) === true))){
                 ...
                 previous_attr_id = order_info.attribute_id;
                         printer_conn
                             ...
                             .align('lt')
                                ...
                                attr_name_global = attribute_name;
        ...
        if (order_info.menuItem == '???PRINT_LINE_1???') {
            ...
            console.log('order_info.menuItem', order_info.menuItem);

                printer_conn
                
                .size(template_styles[template_style-1].size_1, template_styles[template_style-1].size_2)
                
                .text(template_styles[template_style-1].tab_qty+order_info.quantity+template_styles[template_style-1].item_new_line+template_styles[template_style-1].tab_item+ (order_info.menuItem).trim() +template_styles[template_style-1].tab_item_2)
        ...
        if (order_info.menuExtrasSelected !== undefined && order_info.menuExtrasSelected.length > 0) {
            ...
            order_info.menuExtrasSelected.forEach(function(extras) {
                ...
                if (order_data.extra_category_title_print) {
                    ...
                    printer_conn
                    .text(template_styles[template_style-1].tab_extras+'(' + (extras.heading).trim() + ')')
            ...
            extras.optionNameSelected.forEach(function(options) {
                ...
                printer_conn
                .text(template_styles[template_style-1].tab_extras+'> ' + (options.name).trim())
...
if (show_other_stations === true) {
    ...
    if (print_data_other.length > 0) {
        ...
        print_data_other.forEach(function (order_info_other) {
            ...
            if (order_info_other.menuItem != '???PRINT_LINE_1???') {

                if(!isItemIdInArray(attr_id_main, order_info_other.attribute_id, runner_flag)){
                printer_conn
                    ...
                    if (order_info_other.menuExtrasSelected !== undefined && order_info_other.menuExtrasSelected.length > 0 && show_other_stations_extras === true) { 
            
                        order_info_other.menuExtrasSelected.forEach(function(extras) {
            
                            if (order_info_other.extra_category_title_print) {
...
function isItemIdInArray(array, item, is_runner = false) {
